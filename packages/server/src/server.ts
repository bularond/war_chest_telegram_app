 ...
 *
 * Under `ROLE=app` it is not this process's job at all. Telegram hands updates
 * to one poller per token, so the second one to ask gets `Conflict` and the two
 * of them take turns losing greetings — which is why the role decides this
 * rather than the presence of a variable somebody may have copied across.
 */
if (config.role === 'app') {
  if (config.publicUrl) {
    app.log.info('ROLE=app — chat is another process; PUBLIC_URL is unused here');
  }
} else if (config.botToken && config.publicUrl) {
...
app.log.info({ role: config.role }, 'War Chest is up');
if (config.devAuth) {
